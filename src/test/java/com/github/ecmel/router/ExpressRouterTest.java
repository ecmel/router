...
public class ExpressRouterTest
{
    ...
    static
    {
        ...
        x.put("/trades/:commoditySlug/:date(\\d{2}-\\d{2}-\\d{4})?",
            new String[] { "/trades/porkbelly", "/trades/porkbelly/07-06-2017" });
        x.put("/trades/:commoditySlug/:optionId/:date?",
            new String[] { "/trades/porkbelly/2000", "/trades/porkbelly/2000/07-06-2017" });
        ...
        x.put("/:commoditySlug/straddles/:straddleSlug?",
            new String[] { "/porkbelly/straddles", "/porkbelly/straddles/porkbelly-euro" });
        x.put("/:commoditySlug/straddles/:straddleSlug/bid/:bidId",
            new String[] { "/porkbelly/straddles/porkbelly-euro/bid/1" });
    ...
    public void testMultipleMatchingPatternsCompareMostSpecific()
    {
        ...
        final List<String> sorted = allPathDefinitions.keySet().stream().map(ExpressRoute::new)
            .filter(route -> route.matches(test)).sorted((a, b) -> {
                ExpressRoutePatternComparator foo = new ExpressRoutePatternComparator(test);
                return foo.compare(a, b);
            }).map(ExpressRoute::getPathDefinition).collect(Collectors.toList());
